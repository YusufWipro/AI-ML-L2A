...
filename =  r'iris.csv'
names = ['sepal_length','sepal_width','petal_length','petal_width','class_flower']
...
encode = LabelEncoder()
Target = encode.fit_transform(dataframe.iloc[:,4])
dataframe['class']=Target
print(dataframe.head())

# class_flower is the classifier and rest are predictor features
x = np.array(dataframe.drop(['class_flower'],1))
y = np.array(dataframe.class_flower)
